...
class U:
    ...
    def compound_label(v: Any) -> str:
        ...
        return "日次複利" if s == AppConfig.COMPOUND["DAILY"] else "月次複利" if s == AppConfig.COMPOUND["MONTHLY"] else "単利"
...
class Repository:
    ...
    def repair_settings(self, settings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            if not repaired[AppConfig.HEADERS["SETTINGS"]].astype(str).reset_index(drop=True).equals(settings_df.reindex(columns=AppConfig.HEADERS["SETTINGS"]).astype(str).reset_index(drop=True)):
                need_write = True
        except Exception:
            need_write = True
...
class AppUI:
    ...
    def render_apr(self, settings_df: pd.DataFrame, members_df: pd.DataFrame) -> None:
        ...
        project_net_factor, compound_timing = float(row.get("Net_Factor", AppConfig.FACTOR["MASTER"])), U.normalize_compound(row.get("Compound_Timing", AppConfig.COMPOUND["NONE"]))
        ...
        if st.button("Ledgerテスト書き込み"):
            try:
                ...
                self.repo.append_ledger(ts, "TEST", "TEST_USER", AppConfig.TYPE["APR"], 1.2345, "TEST_WRITE")
                self.repo.gs.clear_cache()
                ledger_df_after = self.repo.load_ledger()
                self.repo.write_apr_summary(self.engine.build_apr_summary(ledger_df_after, members_df))
                self.repo.gs.clear_cache()
                st.success(f"テスト書き込み成功: {self.repo.gs.names.LEDGER} / {self.repo.gs.names.APR_SUMMARY}")
                st.rerun()
            except Exception as e:
        ...
        with st.expander("個人別の本日配当（確認）", expanded=False):
            ...
            show["Principal"], show["DailyAPR"] = show["Principal"].apply(U.fmt_usd), show["DailyAPR"].apply(U.fmt_usd)
        ...
        if st.button("APRを確定して全員にLINE送信"):
            try:
                ...
                for _, r in mem.iterrows():
                    person, uid, disp, daily_apr = str(r["PersonName"]).strip(), str(r["Line_User_ID"]).strip(), str(r["LINE_DisplayName"]).strip(), float(r["DailyAPR"])
                ...
                for _, r in mem.iterrows():
                    person, uid, disp, daily_reward = str(r["PersonName"]).strip(), str(r["Line_User_ID"]).strip(), str(r["LINE_DisplayName"]).strip(), float(r["DailyAPR"])
                    personalized_msg = (
                        "🏦【APR収益報告】\n"
                        f"{person} 様\n"
                        f"プロジェクト: {project}\n"
    ...
    def render_cash(self, settings_df: pd.DataFrame, members_df: pd.DataFrame) -> None:
        ...
        if st.button("確定して保存＆個別にLINE通知"):
            try:
                ...
                new_balance, ts = (current + float(amt) if typ == AppConfig.TYPE["DEPOSIT"] else current - float(amt)), U.fmt_dt(U.now_jst())

                for i in range(len(members_df)):
                    if members_df.loc[i, "Project_Name"] == str(project) and str(members_df.loc[i, "PersonName"]).strip() == str(person).strip():
                        members_df.loc[i, "Principal"], members_df.loc[i, "UpdatedAt_JST"] = float(new_balance), ts
                ...
                token, uid = ExternalService.get_line_token(AdminAuth.current_namespace()), str(row["Line_User_ID"]).strip()
                msg = (
                    "💸【入出金通知】\n"
                    f"{person} 様\n"
                    f"プロジェクト: {project}\n"
    ...
    def render_admin(self, settings_df: pd.DataFrame, members_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not view_all.empty:
            ...
            show = view_all.copy()
            show["Principal"], show["状態"] = show["Principal"].apply(U.fmt_usd), show["IsActive"].apply(U.bool_to_status)
        ...
        if view_all.empty:
            ...
            def label_row(r: pd.Series) -> str:
                name, disp, uid, stt = str(r.get("PersonName", "")).strip(), str(r.get("LINE_DisplayName", "")).strip(), str(r.get("Line_User_ID", "")).strip(), U.bool_to_status(r.get("IsActive", True))
            ...
            if do_send:
                if not selected:
                    ...
                    for lab in selected:
                        ...
                        uid, person_name, disp = str(r.get("Line_User_ID", "")).strip(), str(r.get("PersonName", "")).strip(), str(r.get("LINE_DisplayName", "")).strip()
        ...
        if line_users:
            ...
            if picked != "（選択しない）":
                ...
                st.session_state["prefill_line_uid"], st.session_state["prefill_line_name"] = uid, name
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("🔓 ログアウト", use_container_width=True):
            st.session_state["admin_ok"], st.session_state["admin_name"], st.session_state["admin_namespace"] = False, "", ""
            st.rerun()
    ...
    repo, engine, ui = Repository(gs), FinanceEngine(), AppUI(Repository(gs), FinanceEngine())
